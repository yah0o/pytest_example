...
class TestGetAccountCommerceInfo(object):
    ...
    def test_get_account_commerce_info_should_succeed_when_account_payment_type_is_not_bound(self, config,
                                                                                             content_type):
        ...
        assert_that(payment_methods, has_key('linked'))
        assert_that(payment_methods['linked'], has_length(0))
        assert_that(payment_methods, has_key('non_linked'))
        assert_that(payment_methods['non_linked'], greater_than(0))
    ...
    def test_get_account_commerce_info_should_succeed_when_account_payment_type_is_bound(self, config,
        ...
        payment_methods = commerce_response.content['body']['payment_methods']
        assert_that(payment_methods, has_key('non_linked'))
        assert_that(payment_methods['non_linked'], greater_than(
            0))
        assert_that(payment_methods, has_key('linked'))
        assert_that(payment_methods['linked'], has_length(1))
        assert_that(payment_methods['linked'][0], has_key('name'))
        assert_that(payment_methods['linked'][0]['name'], equal_to(PurchaseUtil.PaymentCode.BRAINTREE_PAYPAL))

    @pytest.allure.severity(severity_level.MINOR)
    @pytest.mark.parametrize('invalid_profile_id', [-1, 0, 0.50, None, ''])
    def test_get_account_commerce_info_should_fail_when_profile_id_is_invalid(self, config, invalid_profile_id):
        commerce_response = config.freya.server_gateway.get_account_commerce_info(invalid_profile_id)
        commerce_response.expect_failure(result_code='ERROR')

    @pytest.allure.severity(severity_level.MINOR)
    def test_get_account_commerce_info_should_fail_when_profile_id_is_a_string(self, config):
        commerce_response = config.freya.server_gateway.get_account_commerce_info('bad_profile')
